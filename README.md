...
2. 下载[tools/nssm.exe](https://raw.githubusercontent.com/xykt/komari-server-win7_win8_legacy/refs/heads/main/tools/nssm.exe)
3. 打开cmd命令行，```cd 'C:\Program Files\Komari'```进入目录，执行```./komari.exe server -l 0.0.0.0:[服务监听端口]```（注意替换你的端口），在出现的一系列文字中，找到“Username: admin , Password: Y34ekYb0unvM”文字，记录（注意Password后面如果出现箭头⬅请不要拷贝箭头和右边的内容），Ctrl+C结束进程
